...
substitutions in I<some> debhelper config files (particularly,
the configuration files for L<dh_install>).
...
If you need a literal dollar sign that cannot trigger an substitution,
...
When in doubt, the B<DEB_HOST_*> variant is the that will work both
...
as it will not matter for most case.
...
they expand beyond an a certain size (4096 characters or 3x length of
the original input - whichever is more).
...
support these.  Remember to be careful if your generator I<also> provide
...
manpage from F<< debian/I<package>.manpages >> (assuming both version
are identical).
...
obsolete command (e.g. B<dh_systemd_enable>) are present in
F<debian/rules>.

=item -

The B<dh_missing> command will now default to B<--fail-missing>.  This
can be reverted to a (non-fatal) warning (like in compat 12) by
explicitly passing B<--list-missing>.
...
B<${foo}> syntax.  In many cases, this can be used to reference
paths that contain either spaces or L<dpkg-architecture(1)> values.

While this can reduce the need for L<dh-exec(1)> in some cases, it
is B<not> a replacement L<dh-exec(1)> in general (such as the
filtering syntax, renaming, etc.).
...
relevant logic I<out> of those targets.
