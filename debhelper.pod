...
compatibility. To use the that method, the F<debian/compat> file should contain
...
Note that you must use either the the build-dependency on debhelper-compat or
...
temporarily either upgrading a few commands to a new compat level or keep a
few commands on a lower compat level.  The feature is best using sparingly
as it effectively introduces special-cases into the F<debian/rules> file
that may be surprising to maintainers or reviewers (or, in the long term,
to yourself).
...
list has moved to L<debhelper-compat-list(7)>.
